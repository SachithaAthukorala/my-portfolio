...
export default function AdminSkillsPage() {
  ...
  function setGroup(i: number, k: keyof typeof d.skills[0], v: string) {
    setD((p: any) => { const a = [...p.skills]; a[i] = { ...a[i], [k]: v }; return { ...p, skills: a } })
  }

  function addGroup() {
    setD((p: any) => ({ ...p, skills: [...p.skills, { category: 'New Group', color: 'accent', items: [] }] }))
  }

  function removeGroup(i: number) {
    setD((p: any) => ({ ...p, skills: p.skills.filter((_: any, j: number) => j !== i) }))
  }

  function addItem(gi: number) {
    const val = (newItem[gi] || '').trim()
    if (!val) return
    setD((p: any) => { const a = [...p.skills]; a[gi] = { ...a[gi], items: [...a[gi].items, val] }; return { ...p, skills: a } })
    setNewItem((p: any) => ({ ...p, [gi]: '' }))
  }

  function removeItem(gi: number, ii: number) {
    setD((p: any) => { const a = [...p.skills]; a[gi] = { ...a[gi], items: a[gi].items.filter((_: any, j: number) => j !== ii) }; return { ...p, skills: a } })
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px,1fr))', gap: 16 }}>
        {d?.skills?.map((group: any, gi: number) => (
          <Card key={gi}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                value={newItem[gi] || ''}
                onChange={e => setNewItem((p: any) => ({ ...p, [gi]: e.target.value }))}
